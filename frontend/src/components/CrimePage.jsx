          ...
          <section className="flex min-h-0 flex-col gap-4 overflow-hidden">
            <div className="grid gap-3 md:grid-cols-2 2xl:grid-cols-3">
              {summaryCards.map((card) => (
                <article
                  key={card.label}
                  className="rounded-[24px] border border-white/5 bg-[#030b0e]/90 p-4 shadow-2xl"
                >
                  <p className="text-[11px] uppercase tracking-[0.3em] text-cyan-100/45">
                    {card.label}
                  </p>
                  <p className={`mt-4 text-2xl font-semibold leading-tight ${card.accent}`}>
                    {card.value}
                  </p>
                  <p className="mt-2 text-sm text-cyan-100/60">{card.meta}</p>
                </article>
              ))}
            </div>
