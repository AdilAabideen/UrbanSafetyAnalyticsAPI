...
def test_initialize_database_creates_tables_indexes_and_admin_seed(monkeypatch):
    ...
    assert any("CREATE TABLE IF NOT EXISTS analytics_risk_score_snapshots" in sql for sql in statements)
    assert any("CREATE TABLE IF NOT EXISTS analytics_risk_forecast_snapshots" in sql for sql in statements)
    assert any("CREATE TABLE IF NOT EXISTS analytics_hotspot_stability_snapshots" in sql for sql in statements)
    ...
    assert any("CREATE INDEX IF NOT EXISTS analytics_risk_score_snapshots_created_idx" in sql for sql in statements)
    assert any("CREATE INDEX IF NOT EXISTS analytics_risk_forecast_snapshots_created_idx" in sql for sql in statements)
    assert any("CREATE INDEX IF NOT EXISTS analytics_hotspot_stability_snapshots_created_idx" in sql for sql in statements)
