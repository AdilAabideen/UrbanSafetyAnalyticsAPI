...
def test_initialize_database_creates_tables_indexes_and_admin_seed(monkeypatch):
    ...
    assert any("CREATE TABLE IF NOT EXISTS watchlist_reports" in sql for sql in statements)
    ...
    assert any("CREATE INDEX IF NOT EXISTS watchlist_reports_watchlist_created_idx" in sql for sql in statements)
