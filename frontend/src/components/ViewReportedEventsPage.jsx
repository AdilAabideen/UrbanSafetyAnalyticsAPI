...
const LOCATION_SOURCE_ID = "view-reports-location";
const LOCATION_HALO_LAYER_ID = "view-reports-location-halo";
const LOCATION_POINT_LAYER_ID = "view-reports-location-point";
const ROADS_SOURCE_ID = "view-reports-roads";
const ROADS_LAYER_ID = "view-reports-roads-layer";

function ViewReportedEventsPage({
  docsUrl,
  accessToken,
  selectedReportId,
  initialReport,
  ...
  useEffect(() => {
    ...
    const loadReports = async () => {
      ...
      try {
        const response = await reportedEventsService.getMyReportedEvents(
          {
        ...
        if (selectedReportId) {
          const matchedReport =
            response.items.find((report) => report.id === selectedReportId) ||
            null;
        ...
        if (initialReport?.id === selectedReportId && selectedReportId) {
        ...
        if (response.items.length) {
          ...
          if (!selectedReportId) {
            onSelectReport?.(response.items[0]);
          }
          return;
        }
        ...
        setReports([]);
        setMeta(null);
        setSelectedReport(initialReport || null);
        setListErrorMessage(error?.message || "Failed to load your reports.");
      } finally {
        if (!controller.signal.aborted) {
  ...
  useEffect(() => {
    ...
    const matchedReport = reports.find((report) => report.id === selectedReportId) || null;
  ...
  const summaryCards = useMemo(() => {
    ...
    const approvedCount = reports.filter((report) => report.moderationStatus === "approved").length;
    const rejectedCount = reports.filter((report) => report.moderationStatus === "rejected").length;
  ...
  if (!accessToken) {
    return (
      <div className="flex min-h-0 flex-1 flex-col bg-[#071316]">
        <TopBar
          docsUrl={docsUrl}
          title="My Reported Events"
          subtitle="Sign in to review reports submitted through the public reporting flow."
        />
        <div className="grid min-h-0 flex-1 place-items-center p-6">
          <div className="w-full max-w-[520px] rounded-[26px] border border-white/5 bg-[#030b0e]/90 p-8 text-center shadow-2xl">
            ...
            <p className="mt-3 text-sm leading-6 text-cyan-100/60">
              Log in first so the viewer can call the authenticated `/reported-events/mine` endpoint.
            </p>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col bg-[#071316]">
      <TopBar
        docsUrl={docsUrl}
        title="My Reported Events"
        subtitle="Review your submitted crime and collision reports, check moderation status, and reopen each location."
      />

      <div className="min-h-0 flex-1 p-4">
        <div className="grid h-full min-h-0 gap-4 xl:grid-cols-[380px,minmax(0,1fr)]">
          <aside className="flex min-h-0 flex-col overflow-hidden rounded-[26px] border border-white/5 bg-[#030b0e]/90 shadow-2xl">
            <div className="flex items-center justify-between gap-3 border-b border-white/5 px-5 py-4">
              <div>
                <p className="text-[11px] uppercase tracking-[0.3em] text-cyan-100/40">My Reports</p>
            ...
            <div className="grid gap-2 border-b border-white/5 px-4 py-4 md:grid-cols-2">
              <FilterSelect
                label="Status"
                value={statusFilter}
            ...
            <div className="min-h-0 flex-1 overflow-y-auto px-4 py-4">
              {reports.length ? (
                ...
                <div className="grid min-h-full place-items-center rounded-[20px] border border-white/5 bg-[#071316]/60 p-6 text-center">
                  <div>
                    <p className="text-lg font-semibold text-cyan-50">No submitted reports yet</p>
                    <p className="mt-2 text-sm text-cyan-100/60">
                      Submit a crime or collision report to see it appear here.
                    </p>
                  </div>
                </div>
              )}
            ...
            {selectedReport ? (
                  ...
                  <div className="mt-4 grid gap-4 lg:grid-cols-2">
                    ...
                    <DetailCard
                      ...
                      rows={[
                        ...
                        ["Notes", selectedReport.moderationNotes || "No moderation notes"],
                      ]}
                    />
                    <DetailCard
                      title="Location"
              ...
              <div className="grid min-h-0 flex-1 place-items-center rounded-[26px] border border-white/5 bg-[#030b0e]/90 p-6 shadow-2xl">
                <div className="text-center">
                  ...
                  <p className="mt-2 text-sm text-cyan-100/60">
                    Pick a report from the left to inspect its moderation state and location.
                  </p>
                </div>
              </div>
            )}
...
export default ViewReportedEventsPage;
